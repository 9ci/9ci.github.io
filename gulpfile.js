...
/**
 * Task: build:scripts:npmcopy
 *
 * Copies scripts from node_modules
 */
...
/**
 * Task: build:styles:npmcopy
 *
 * Copies styles from node_modules
 */
gulp.task('build:styles:npmcopy', function () {
    return gulp.src(
        [
            'node_modules/slick-carousel/slick/slick.scss',
            'node_modules/slick-carousel/slick/slick-theme.scss'
        ])
        .pipe(gulp.dest(paths.sassFiles + '/scss/00-lib/slick'));
});

/**
 * Task: build:fonts:npmcopy
 *
 * Copies fonts from node_modules
 */
gulp.task('build:fonts:npmcopy', function () {
    return gulp.src(
        [
            'node_modules/slick-carousel/slick/fonts/*'
        ])
        .pipe(gulp.dest(paths.fontFiles + '/slick'));
});
...
gulp.task('clean:styles', function(callback) {
    del([paths.jekyllCssFiles, paths.siteCssFiles, '_includes/css/critical.css']);
    callback();
});
 ...
 * Copies fonts.
 */
gulp.task('build:fonts', ['fontawesome','slickfonts']);
...
gulp.task('slickfonts', function() {
    return gulp.src(paths.fontFiles + '/slick/**.*')
        .pipe(rename(function(path) {path.dirname = '';}))
        .pipe(gulp.dest(paths.jekyllFontFiles))
        .pipe(gulp.dest(paths.siteFontFiles))
        .pipe(browserSync.stream())
